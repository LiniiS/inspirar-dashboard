...
def mostrar_diarios_semanais(pacientes_recorte):
    ...
    st.info("Esta versão mostra o comportamento semanal: para cada período, calcula a média de registros de diários de sintomas considerando apenas os usuários que já estavam cadastrados naquele período.")
        ...
        for paciente in pacientes_recorte:
            ...
            if data_cadastro:
                ...
                if data_cadastro <= fim_semana:
                    ...
                    # Calcular registros de diários nesta semana específica
                    ...
                    for diary in diaries:
                        ...
                        if data_diary:
                            ...
                            elif not isinstance(data_diary, pd.Timestamp):
                                continue
                            
                            # Converter para pandas Timestamp se for datetime.datetime
                            if not isinstance(data_diary, pd.Timestamp):
                                data_diary = pd.Timestamp(data_diary)
                            
                            # Normalizar timezone para UTC
                            if data_diary.tz is None:
                                data_diary = data_diary.tz_localize('UTC')
                            else:
                                data_diary = data_diary.tz_convert('UTC')
